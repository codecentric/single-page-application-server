# Helm Chart for Single Page Application Server

## Overview

This Helm chart deploys a Single Page Application (SPA) Server based on Nginx. It provides a scalable and secure way to serve your SPA with customizable configuration options.

## Installation

You can install the chart with the release name `my-release` using the Docker Hub repository.

### Using the `latest` Tag

To install the chart using the `latest` version from Docker Hub:

```sh
helm install my-release oci://docker.io/codecentric/single-page-application-server --version latest
```

### Using a Specific Version (`{MAJOR}.{MINOR}.{PATCH}`) Tag

To install a specific version of the Helm chart from Docker Hub, replace `{MAJOR}.{MINOR}.{PATCH}` with the desired version number of this chart:

```sh
helm install my-release oci://docker.io/codecentric/single-page-application-server \
